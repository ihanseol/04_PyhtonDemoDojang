...
class Coordinates:
    def __init__(self):
        ...
        self.origin_meridian_mo = self.calculate_origin_meridian_mo()
    ...
    def calculate_origin_meridian_mo(self):
        return self.semi_minor_axis_a * (
    ...
    def convert_to_plane_rect(self, latitude, longitude):
        ...
        x = self.origin_addition_value_y + self.origin_scale_factor_ko * n * (
            a + (a ** 3) / 6 * (1 - t + c) + (a ** 5) / 120 * (5 - 18 * t + t ** 2 + 72 * c - 58 * self.second_eccentricity)
        )
        y = self.origin_addition_value_x + self.origin_scale_factor_ko * (
            m - self.origin_meridian_mo + n * math.tan(phi) * (
                (a ** 2) / 2 + (a ** 4) / 24 * (5 - t + 9 * c + 4 * (c ** 2)) + (a ** 6) / 720 * (61 - 58 * t + t ** 2 + 600 * c - 330 * self.second_eccentricity)
            )
        )
        return x, y
    ...
    def convert_to_latitude_longitude(self, tm_x, tm_y):
        ...
        phi1 = mu1 + (3 * self.origin_meridian_e1 / 2 - 27 * (self.origin_meridian_e1 ** 3) / 32) * math.sin(2 * mu1) + \
                (21 * (self.origin_meridian_e1 ** 2) / 16 - 55 * (self.origin_meridian_e1 ** 4) / 32) * math.sin(4 * mu1) + \
                (151 * (self.origin_meridian_e1 ** 3) / 96) * math.sin(6 * mu1) + \
                (1097 * (self.origin_meridian_e1 ** 4) / 512) * math.sin(8 * mu1)
        ...
        phi = (phi1 - (n1 * math.tan(phi1) / r1) * (
            (d ** 2) / 2 - (d ** 4) / 24 * (5 + 3 * t1 + 10 * c1 - 4 * (c1 ** 2) - 9 * self.second_eccentricity) +
            (d ** 6) / 720 * (61 + 90 * t1 + 298 * c1 + 45 * (t1 ** 2) - 252 * self.second_eccentricity - 3 * (c1 ** 2))
        )) * 180 / self.pi
        lamda = self.origin_longitude + ((1 / math.cos(phi1)) * (d - (d ** 3) / 6 * (1 + 2 * t1 + c1) + (d ** 5) / 120 * (5 - 2 * c1 + 28 * t1 - 3 * (c1 ** 2) + 8 * self.second_eccentricity + 24 * (t1 ** 2)))) * 180 / self.pi + self.correction_10405_seconds
        return phi, lamda

    def convert_to_grid(self, latitude, longitude):
        ra = self.re * self.sf / (math.tan(self.pi * 0.25 + latitude * self.degrad * 0.5) ** self.sn)
    ...
    def convert_to_latitude_longitude_from_grid(self, nx, ny):
        ...
        ra = math.sqrt(xn ** 2 + yn ** 2)
        if self.sn < 0.0:
            ra = -ra
        alat = 2.0 * math.atan((self.re * self.sf / ra) ** (1.0 / self.sn)) - self.pi * 0.5
        theta = 0.0
        if abs(xn) <= 0.0:
            theta = 0.0
        else:
