...
class Coordinates:
    def __init__(self):
        ...
        self.origin_addition_value_x = 500000.0
        self.origin_addition_value_y = 200000.0
        self.origin_latitude = 38.0
        self.origin_longitude = 127.0
        self.first_eccentricty = (self.semi_minor_axis_a ** 2 - self.semi_minor_axis_b ** 2) / self.semi_minor_axis_a ** 2
        self.second_eccentricty = (self.semi_minor_axis_a ** 2 - self.semi_minor_axis_b ** 2) / self.semi_minor_axis_b ** 2
    ...
    def convert_to_plane_rect(self, latitude, longitude):
        ...
        a = lamda - (self.origin_longitude / 180 * self.pi)
        ...
        y = self.origin_addition_value_x + self.origin_scale_factor_ko * (m - self.origin_meridian_mo + n * math.tan(phi) * (math.pow(a, 2) / 2 + math.pow(a, 4) / 24 * (5 - t + 9 * c + 4 * math.pow(c, 2)) + math.pow(a, 6) / 720 * (61 + 90 * t + 298 * c + 45 * math.pow(t, 2) - 252 * self.second_eccentricty - 3 * math.pow(c, 2))))
        return (x, y)
    ...
    def convert_to_latitude_longitude_grid(self, nx, ny):
        ...
        if self.sn < 0.0:
            ra = -ra
        alat = 2 * math.atan(math.pow(self.re * self.sf / ra, 1 / self.sn)) - self.pi * 0.5
        theta = 0.0
        if abs(xn) <= 0.0:
            theta = 0.0
        else:
...
latitude, longitude = coordinates.convert_to_plane_rect(37.5665, 126.9779)
