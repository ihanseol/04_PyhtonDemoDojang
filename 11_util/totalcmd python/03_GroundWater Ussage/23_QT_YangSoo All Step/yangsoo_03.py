...
from YangSoo.FileProcessing_V3 import FileBase
from YangSoo.FileProcessing_V3 import PathChecker
...
class AQTProcessor(AQTBASE):
    ...
    def aqtesolver_main(self, file_name) -> object:
...
class RunTimeTimer:
    def __init__(self, seconds=10):
        self.sec = seconds
        self.program_name = r"c:\Program Files\totalcmd\ini\02_python\tkinter_timer2.py"
...
class InjectValueToSheet(FileProcessing):
    ...
    def injecttion_long_test_01(self, well, ws_janggi, ws_skin, mode='auto'):
        self.printinfo('='*100)
    ...
    def injecttion_long_test_02(self, well, ws_janggi, ws_skin, mode='auto'):
        self.printinfo('='*100)
    ...
    def injection_recover(self, well, ws_recover, ws_skin, mode='auto'):
        ...
        if mode == 'mannual':
            ...
            val_t, val_s, val_x = get_ts.aqtesolver_main(file_name=self.SEND + f"w{well}_03_recover.aqt")
        else:
    ...
    def main_process(self, well, mode=1) -> None:
        ...
        if (mode == 2 or mode == 3) and mode != 4:
            self.injecttion_long_test_01(well, ws_janggi, ws_skin, mode='mannual')
            self.injecttion_long_test_02(well, ws_janggi, ws_skin, mode='mannual')
            self.injection_recover(well, ws_recover, ws_skin, mode='mannual')
        else:
            self.injecttion_long_test_01(well, ws_janggi, ws_skin, mode='auto')
            self.injecttion_long_test_02(well, ws_janggi, ws_skin, mode='auto')
...
class PumpTestAutomation(FileProcessing):
    ...
    def main_processing(self, Mode=1):
        ...
        self.countdown(3)
        # self.window_to_secondscreen("Pump Test Automation")
