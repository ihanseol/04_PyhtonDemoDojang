...
class SimpleWindowMover:
    ...
    def move_window(self):
        found_windows = self.find_window_by_title("minhwasoo")

        hwnd = found_windows[-1][0]
        title = found_windows[-1][1]

        print(f"'{hwnd} : hwnd, {title}' 창을 이동합니다...")


        if hwnd:
            print(f"'{title}' 창을 이동합니다...")
            if self.move_window_to_second_monitor(hwnd):
                print("이동 완료!")
            else:
                print("이동 실패!")
        else:
