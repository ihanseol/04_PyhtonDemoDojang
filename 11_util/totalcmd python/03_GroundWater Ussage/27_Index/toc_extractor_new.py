...
class TocExtractor:
    """HWP 문서에서 목차 항목별 페이지 번호를 추출한다.
    HWP 인스턴스를 외부에서 전달받아 사용한다.
    """

    def __init__(self, file_path: str):
        self.file_path = file_path
        # self._hwp 멤버변수 제거 → 외부에서 전달받음

    # ── 공개 인터페이스 ──────────────────────────

    def extract(self, hwp: Hwp, target_list: list[str]) -> dict[str, int]:
        """
        target_list 를 순회하며 각 텍스트가 위치한 페이지 번호를 반환한다.

        Args:
            hwp: 외부에서 생성·관리되는 HWP 인스턴스  ← 추가!
        """
        hwp.open(self.file_path)               # ← 전달받은 hwp 사용
        results: dict[str, int] = {}
        total = len(target_list)

        print(f"--- 파일 분석 시작: {self.file_path} ---")
        # try/finally 제거 → hwp.quit()를 외부에서 관리
        for i, target in enumerate(target_list):
            self._scan_text(hwp, target)        # ← hwp 인자 전달
            progress_bar(i + 1, total, prefix='Progress:', suffix='Complete', length=50)
            results[target] = self._current_page(hwp)

        return results

    # ── 내부 메서드 ──────────────────────────────

    @staticmethod                              # ← hwp를 인자로 받음
    def _current_page(hwp: Hwp) -> int:
        return hwp.KeyIndicator()[3]

    @staticmethod                              # ← hwp를 인자로 받음
    def _scan_text(hwp: Hwp, search_text: str) -> set[int]:
        pages: set[int] = set()
...
class TocWriter:
    ...
    def write(self, hwp: Hwp, page_numbers: list[int]):
        """
        Args:
            hwp: 외부에서 생성·관리되는 HWP 인스턴스  ← 추가!
        """
        print("-- Enter TocWriter.write() --", flush=True)
        hwp.open(self.toc_file_path)            # ← 전달받은 hwp 사용
...
class TocPipeline:
    """HWP 인스턴스를 한 번만 생성하여 전체 파이프라인에서 공유한다."""

    def __init__(self, source_toc, dest_toc, source_dir, toc_targets,
                 hwp_extension="hwp", visible=False):
    ...
    def run(self):
        ...
        try:
            ...
            toc_results = extractor.extract(hwp, self.toc_targets)  # ← hwp 전달
            ...
            writer.write(hwp, list(toc_results.values()))           # ← hwp 전달
        finally:
            hwp.quit()  # ★ 한 번만 종료
