...
def merge_aassii_files(ofilename='01_취합본.hwp'):
    hwp = Hwp(visible=False)  # 한/글 실행
    fb = FileBase()
    ...
    file_list = ["ss_report.hwp", "aa_report.hwp", "ii_report.hwp", ]
    print(file_list)
...
def remove_hwpx_files():
    """Removes all *.hwpx files within the specified directory."""
    directory = "D:\\05_Send"
    pattern = os.path.join(directory, "*.hwpx")
    hwpx_files = glob.glob(pattern)

    if not hwpx_files:
        print(f"No *.hwpx files found in: {directory}")
        return

    for filepath in hwpx_files:
        try:
            os.remove(filepath)
