    ...
    def move_documents_to_ihanseol(self):
        ...
        if self.isDIRSET:
            ...
            for f in dat_files:
                ...
                target = self.join_path_tofilename(self.DIR_PRN, f)
                fb.move_file(source, target)

            for f in xlsm_files:
                source = self.join_path_tofilename(self.DOCUMENTS, f)
                target = self.join_path_tofilename(self.DIR_YANGSOO_TEST, f)
                fb.move_file(source, target)
    ...
    def move_send_to_ihanseol(self):
        ...
        if len(a_pdffiles) > 0 and len(jpg_apagefiles) > 0:
            ...
            for f in a_pdffiles:
                ...
                target = self.join_path_tofilename(self.DIR_AQT, f)
                fb.move_file(source, target)
        ...
        if len(jpg_wpagefiles) > 0 and len(w_pdffiles) > 0:
            ...
            for f in jpg_wpagefiles:
                source = self.join_path_tofilename(self.SEND, f)
                target = self.join_path_tofilename(self.DIR_YANGSOOILBO, f)
                fb.move_file(source, target)
