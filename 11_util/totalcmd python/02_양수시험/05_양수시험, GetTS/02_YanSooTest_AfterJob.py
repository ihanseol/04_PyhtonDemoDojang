...
def pi_info(message):
    def show_message():
        # Create a new Tkinter root window for the message box
        root = tk.Tk()
    ...
    # Run the message box in a separate thread to make it process-independent
...
class InjectValueToSheet(FileProcessing):
    ...
    def injecttion_step(self, well, ws_step):
        ...
        val_t, val_s, val_x = get_ts.aqtesolver_main(file_name=self.SEND + f"w{well}_01_step.aqt")
