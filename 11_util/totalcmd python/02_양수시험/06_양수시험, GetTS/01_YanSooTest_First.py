...
class YangSooInjector:
    ...
    def get_data_by_gong(self, gong_number, file_path=r"d:\05_Send\YanSoo_Spec.xlsx"):
        try:
            # Excel 파일을 DataFrame으로 읽어옵니다.
            # read_excel은 인코딩을 명시할 필요가 없으며, sheet_name 인자를 사용합니다.
            try:
                # 첫 번째 시트(sheet_name=0)를 기본값으로 사용
            ...
            result_df = df[df['gong'] == gong_number]
    ...
    def make_cell_values(self, row_index):
        ...
        row_data = self.get_data_by_gong(f"W-{row_index}")

        len_row_data = len(row_data)
        print('len(row_data):', len_row_data)

        address = row_data['address'].iloc[0]
        hp = row_data['hp'].iloc[0]
        casing = row_data['casing'].iloc[0]
        well_rad = row_data['well_diameter'].iloc[0]
        simdo = row_data['simdo'].iloc[0]
        q = row_data['q'].iloc[0]
        natural = row_data['natural'].iloc[0]
        stable = row_data['stable'].iloc[0]

        project_name = jigu_name = company_name = ''
        ph = 7.0

        self.STABLE_TIME = row_data['stable_time'].iloc[0]

        print('time stamp of longtest_time :', row_data['longterm_test_time'].iloc[0])

        # Fixed: Check if timestamp exists and handle timezone properly
        longterm_time = str(row_data['longterm_test_time'].iloc[0])

        self.LONG_TERM_TEST_TIME = longterm_time
        print('time stamp of longtest_time  detail :', self.LONG_TERM_TEST_TIME, type(self.LONG_TERM_TEST_TIME))

        project_name = row_data['Project Name'].iloc[0]
        jigu_name = row_data['Jigu Name'].iloc[0]
        company_name = row_data['Company'].iloc[0]
        pump_simdo = row_data['pump_simdo'].iloc[0]
        tochul = row_data['tochul'].iloc[0]
        ph = row_data['ph'].iloc[0]
        pumping_capacity = row_data['pumping_capacity'].iloc[0]
        ...
        gong = self.extract_number(row_data['gong'].iloc[0])
    ...
    def inject_value_to_cells(self, book, excel):
        ...
        row_index = self.extract_number(filename)
    ...
    def _inject_long_term_test(self, wb, excel):
        ...
        try:
            if self.LONG_TERM_TEST_TIME is None:
                raise ValueError("LONG_TERM_TEST_TIME is None.")
