...
class InjetValueToSheet(FileProcessing):
    def __init__(self):
        super().__init__()
        self.aqt_processor = GetTS.AQTProcessor()
    ...
    def InjecttionStep(self, well, ws_step):
        ...
        time.sleep(1)
        val_T, val_S = self.aqt_processor.AqtesolverMain(file_name=self.SEND + f"w{well}_01_step.aqt")
    ...
    def InjecttionLongTest_01(self, well, ws_janggi, ws_skin):
        ...
        val_T, val_S = self.aqt_processor.AqtesolverMain(file_name=self.SEND + f"w{well}_02_long.aqt")
        print(val_T, val_S)
    ...
    def InjecttionLongTest_02(self, well, ws_janggi, ws_skin):
        ...
        val_T, val_S = self.aqt_processor.AqtesolverMain(file_name=self.SEND + f"w{well}_02_long_01.aqt")
        print(val_T, val_S)
    ...
    def InjectionRecover(self, well, ws_recover, ws_skin):
        ...
        time.sleep(1)
        val_T, val_S = self.aqt_processor.AqtesolverMain(file_name=self.SEND + f"w{well}_03_recover.aqt")
        print(val_T, val_S)
...
class PumpTestAutomation(FileProcessing):
    ...
    def main_processing(self):
        ...
        xlsmfiles = self.get_xlsm_filter(self.SOURCE, sfilter="*_ge_OriginalSaveFile.xlsm")
